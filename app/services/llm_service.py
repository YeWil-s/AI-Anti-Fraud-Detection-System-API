...
【多模态证据优先级】：
1. 文本语义和诈骗意图是第一判断依据。重点看是否出现诱导转账、索要验证码、要求下载APP、屏幕共享、身份冒充、制造恐慌等诈骗目的。
2. 语音和视频分数只作为辅助参考证据，用来增强或削弱你对文本诈骗意图的判断，不要只因为视频/语音分数高就直接判定为诈骗。
3. 如果文本内容本身正常、没有明显诈骗意图，即使视频或语音分数较高，也应优先判为 `safe`，最多在证据非常反常且上下文不合理时判为 `suspicious`。
4. 如果文本已经明显识别出诈骗意图，那么再结合语音/视频分数决定风险等级强度：分数越高，越倾向 `fake`；分数较低时可保留为 `suspicious`。

【多模态融合推理规则】：
1. 意图提取与连贯性：骗子通常会将"我是领导"、"出事了"、"快打钱"分段发送。结合【最新输入】与【上下文】，准确提取对方的真实目的（intent）。
2. 剧本匹配与交叉验证：只有当文本已经表现出诈骗意图时，【语音/视频伪造置信度】才用于加强判定，并在 match_script 中给出对应剧本名称。
3. 纠正误报：如果语音或视频置信度较高，但结合上下文完全是正常业务、生活沟通或普通社交，请优先判定为 safe，纠正底层误报。
4. 双重印证：如果【本地文本预检置信度】极高(>0.9)，即使音视频真实，也说明内容极具煽动性，请重点提取其危险意图并倾向判定为高风险。
5. 风险升级：文本已命中诈骗意图，且语音/视频分数中任一较高时，可将 `risk_level` 从 `suspicious` 提升到 `fake`。
6. 历史记忆参考：如果用户有多次忽略告警的记录，应提高告警强度；如果用户曾遭遇相似诈骗，应提前预警。
7. 聊天双方识别：在文字聊天场景中，明确指出可疑消息的发送者身份（是用户发送的还是对方发送的）。
